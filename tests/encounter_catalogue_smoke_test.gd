...
func _init() -> void:
	...
	var stage_two_one := CATALOGUE.get_final_waves(4, false)
	check(_ids(stage_two_one[0]) == ["frog_monster", "frog_monster", "ent_lvl1"], "2-1 first wave mismatch")
	var stage_two_two := CATALOGUE.get_final_waves(5, false)
	check(_ids(stage_two_two[2]) == ["ent_lvl3", "ent_lvl2", "mushroom"], "2-2 third wave mismatch")
	check(CATALOGUE.get_boss_roster(6).size() == 5, "2-3 boss composition mismatch")
